...
市面上的 Claude Code 状态监控工具（AI Light、AgentTray 等）大多需要**编译二进制文件**、安装 **Node.js / Rust 运行时**，甚至要跑一个 **Web Dashboard 后台服务**——越重越不安全。

这个项目只做一件事，并且做得最干净：

| | 本项目 | AI Light | AgentTray | Code Light |
|---|---|---|---|---|
| 外部依赖 | **零**，Python 标准库 | 编译二进制 exe | Tauri + Node 运行时 | Rust 编译 |
| 网络通信 | 纯本地 127.0.0.1 | 本地 HTTP 服务 | 本地 HTTP 服务 | 无网络（文件轮询）|
| 第三方代码 | **零**，全部可审计 | 闭源 hook.exe | 开源 Rust 项目 | 开源 Rust 项目 |
| 安装步骤 | 拖下来 `python` 直接跑 | 下载安装包 | 编译或下载 | 编译或下载 |
| 代码量 | **~260 行** | 数千行 | 数千行 | 数千行 |
| 数据上传 | **绝不** | 无 | 无 | 无 |

**核心原则：**

- 所有数据只在你电脑的 `127.0.0.1` 转一圈，数据包不出网卡
- 不调第三方 API，不遥测，不上报任何信息
- 一个 Python 文件即全部，每行代码你都能看清
- 唯一的"外部通信"是你 Claude Code 的 http hook 发给本地 127.0.0.1

> 如果你在意"我只想看个灯，不想装一整个运行时环境"，这个就是给你的。
